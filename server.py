...
def Encodings(opath,erpath,jsonpath):
    dict1={}
    progress_bar=tqdm(total=len(dict1),desc='Encoding images')
    f=open(jsonpath,"w")
    error_image_name=[]
...
def extract_count(gpath,epath,filename):
    ...
    if count==0:
        img=PIL.Image.open(gpath)
        l=img.size
        m=max(l)
        img=resizeimage.resize_crop(img,[m,m])
        img.save(gpath,img.format)
        out=PIL.Image.open(gpath)
        out=out.rotate(270)
        out.save(gpath)
        image = face_recognition.load_image_file(gpath)
        ...
        if request.method == 'POST':
            ...
            if valid_mailid:
                try:
                    ...
                    flash("Unable to send email contact Developer")
            else:
...
excel=project+"templates\\excel_html\\"
if not os.path.isdir(str(excel)):
    os.mkdir(str(excel))
...
@app.route('/view_attendance_com',methods=['GET','POST'])
def view_attendance_com():
    try:
        if 'username' in session:
            users = mongo.db.users
            login_user = users.find_one({'username' : session['username']})
            ...
            if request.method == "POST":
                ...
                if(os.path.isdir(check_path)):
                    ...
                    if login_user["username"] == "ADMIN":
                        return render_template("/admin/view_attendance.html",dep=dep,year=year,sub=sub,view_excel=value,date_msg=msg,date=dt,username=name,success=success)    
                    return render_template("details.html",dep="none",year="none",sub=sub,view_excel=value,date_msg=msg,date=dt,username=name)
                else:
                    flash("Invalid details")
                    if login_user["username"] == "ADMIN":
                        return render_template("/admin/view_attendance.html",username=name,fail=alert)
...
def imgupload():
    ...
    if request.method == "POST":
        ...
        if dep=="NONE":
            ...
            filename=str(os.getcwd()+"\\static\\images\\Subject\\"+sub+"\\extracted\\")
            try:
                os.makedirs(filename)
            except:
                print()
            app.config["IMAGE_UPLOADS"]=filename
            filename1=filename+"\\image.jpg"
